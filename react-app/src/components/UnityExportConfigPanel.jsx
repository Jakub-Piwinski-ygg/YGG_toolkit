...
// Shared Unity-export configuration UI. Used both by AssetChecker's ExportPanel
// (which wraps it with an entries-driven "Export to Unity…" action) and by
// the Project Scaffold tool (which uses the settings to build an empty-folder
// Unity-layout template). State lives in UnityExportContext so changes in one
// tool are visible in the other.
export function UnityExportConfigPanel() {
  const { settings, setSettings, setRename, updateMapping, removeMapping, addMapping, moveMapping } = useUnityExport();
